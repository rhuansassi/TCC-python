...
def comp_gabdual_long(g, a, M):
    ...
    gdf = np.zeros((int(p*q*R), int(c*d)), dtype=complex)
    G = np.zeros((int(p), int(q*R)), dtype=complex)
    assert(p == 1)
    assert(q == 1)
    for ii in range(c*d):
        G[:] = gf[:, ii]
...
def comp_wfac(g, a, M):
    ...
    gf = np.zeros((int(p), int(q*R), int(c), int(d)), dtype=complex)
    ...
    for w in range(R):
        for s in range(int(d)):
            for l in range(int(q)):
                indices = (np.arange(c) + np.floor((-l * a + s * p * M) % L)).astype(int)
                gf[0, int(l + q * w), :, s] = g[indices]


    assert(d > 1)
    gf = fft(gf, axis=3)
    gf *= np.sqrt(M)
    return gf.reshape((int(p*q*R), int(c*d)), order='F')
...
def comp_iwfac(gf, L, a, M):
    ...
    q = M / c
    d = N / q
    gf = gf.reshape((int(p), int(q*R), int(c), int(d)), order='F')
    gf /= np.sqrt(M)

    if d > 1:
        gf = ifft(gf, axis=3)

    g = np.zeros((int(L), int(R)), dtype=complex)
    for w in range(int(R)):
        for s in range(int(d)):
            for l in range(int(q)):
                for k in range(int(p)):
                    g[np.arange(c) + np.floor((k * M - l * a + s * p * M) % L).astype(int), w] = gf[k, int(l + q * w), :, s].reshape((c,), order='F')
