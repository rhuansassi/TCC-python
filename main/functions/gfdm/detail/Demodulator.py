...
from gfdm.detail import gfdmutil as util
...
class DefaultDemodulator(Demodulator):
    ...
    def _prepareDemodulation(self):
        if self._kind == 'ZF':
            ...
            self._Gzf = fft(gzf[::int(self._pnew.K/L)], axis=0)
    ...
    def __demodulateInFD(self, signal, G):
        ...
        for k in range(K):
            ...
            carrier = np.roll(Xhat, int(-offset + L*M//2 - M*k))
            # carrier = fftshift(carrier[:L*M])
            carrier = fftshift(carrier[:int(L*M)])
            carrierMatched = carrier * G
            # dhat = ifft(carrierMatched, axis=0)[::L]  # downsampling
            dhat = ifft(carrierMatched, axis=0)[::int(L)]  # downsampling
            Dhat[k, :] = dhat
        return Dhat
