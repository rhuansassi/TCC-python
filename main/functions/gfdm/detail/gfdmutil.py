...
from main.functions.gfdm.detail import filters
from main.functions.gfdm.detail import gabor
...
def get_mset(pnew):
    if hasattr(pnew, 'Mset') and pnew.Mset is not None:
        mset = np.mod(pnew.Mset, pnew.M)
        assert len(mset) <= pnew.M
        return mset
    elif hasattr(pnew, 'Mon') and pnew.Mon is not None:
        print(f"pnew.Mon: {pnew.Mon}, pnew.M: {pnew.M}")  # Adicionado para depuração
        assert pnew.Mon <= pnew.M
        start = int(np.ceil((pnew.M - pnew.Mon) / 2))
        end = pnew.M - int(np.floor((pnew.M - pnew.Mon) / 2))
